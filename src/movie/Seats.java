...
public class Seats {
	public static final int MAX_ROW = 5; // 최대 행 수
    public static final int MAX_COL = 9; // 최대 열 수
    ...
    public Seats(ArrayList<Reservation> reservations) throws Exception {
        for (int i = 0; i < MAX_ROW; i++) {
            for (int j = 0; j < MAX_COL; j++) {
                map[i][j] = "o"; // 초기 좌석 상태 설정(좌석 예매 가능)
            }
        }
    ...
    public void mark(String seatName) throws Exception { // 좌석의 예매 표기
